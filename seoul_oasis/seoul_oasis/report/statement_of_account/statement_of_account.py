...
class ReceivablePayableReport(object):
	...
	def build_data(self):
		...
		for index,(key, row) in enumerate(self.voucher_balance.items()):
			...
			if index == 0:
				cummulative = row["outstanding"]
			else:
				cummulative += row["outstanding"]
			row.test = cummulative
	...
	def get_total_pdc_amount(self, party):
		total_pdc_amount = frappe.db.sql("""
			SELECT SUM(paid_amount)
			FROM `tabPayment Entry`
			WHERE party = %s AND custom_is_pdc = 1 AND mode_of_payment = 'Cheque' AND docstatus = 1
		""", party)[0][0] or 0.0
		return total_pdc_amount
	...
	def append_row(self, row):
		...
		row["terms"] = frappe.db.get_value("Customer", row.party, "payment_terms") or 'terms'
		row["trn"] = frappe.db.get_value("Customer", row.party, "tax_id") or "trn"
		row["address"] = frappe.db.get_value("Customer", row.party, "primary_address") or "primary address"
		row["mobile"] = frappe.db.get_value("Customer", row.party, "mobile_no") or "mobile"
		row["total_pdc_amount"] = self.get_total_pdc_amount(row.party) 
		...
		for item in self.data:
			total_range1 += item.get("range1", 0.0)
			total_range2 += item.get("range2", 0.0)
			total_range3 += item.get("range3", 0.0)
			total_range4 += item.get("range4", 0.0)
			total_invoice_amount += item.get("invoiced", 0.0)
			total_outstanding_amount += item.get("outstanding", 0.0)
		
		row["total_range1"] = total_range1
		row["total_range2"] = total_range2
		row["total_range3"] = total_range3
		row["total_range4"] = total_range4
		row["total_invoice_amount"] = total_invoice_amount
		row["total_outstanding_amount"] = total_outstanding_amount
